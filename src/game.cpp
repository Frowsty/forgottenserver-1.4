...
void Game::playerUseWithCreature(uint32_t playerId, const Position& fromPos, uint8_t fromStackPos, uint32_t creatureId, uint16_t spriteId)
{
	...
	if (!Position::areInRange<Map::maxClientViewportX - 1, Map::maxClientViewportY - 2, 0>(creature->getPosition(), player->getPosition())) {
		return;
	}
	...
	if (!spectatorsPtr || spectatorsPtr->empty()) {
		...
		if (type != TALKTYPE_YELL && type != TALKTYPE_MONSTER_YELL) {
			...
			map.getSpectators(spectators, *pos, true, false,
						...
						Map::maxClientViewportY * 2, Map::maxClientViewportY * 2);
		}
	} else {
...
bool Game::reload(ReloadTypes_t reloadType)
{
	switch (reloadType) {
		...
		case RELOAD_TYPE_VOCATIONS: g_vocations.reload(); break;
