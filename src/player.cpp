...
uint32_t Player::getAttackSpeed() const
{
	...
	uint32_t ret = weapon->getAttackSpeed();
...
void Player::addManaSpent(uint64_t amount)
{
	...
	while ((manaSpent + amount) >= nextReqMana) {
		...
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You advanced to sorcery level {:d}.", magLevel));
...
void Player::removeManaSpent(uint64_t amount, bool notify/* = false*/)
{
	...
	if (notify) {
		...
		if (oldLevel != magLevel) {
			sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You were downgraded to sorcery level {:d}.", magLevel));
			sendUpdateStats = true;
		}
